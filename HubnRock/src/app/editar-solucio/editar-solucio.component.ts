...
export class EditarSolucioComponent implements OnInit, HasUnsavedData {
                      ...
                      this.solucio.recursos.forEach(recurs => {
                        fetch(this.fileStorageUrl + recurs.recurs_solucio_url_file, {
                          ...
                          .then(response => response.blob()).then(blob => {
                            let file = new File([blob], recurs.recurs_solucio_nom);
  ...
  onSubmit() {
    ...
    if (!this.solucioForm.valid) {
      ...
      let confirmWindow = confirm('Està segur que vol actualitzar aquesta solució?')
      ...
      if (confirmWindow == true) {
        ...
        let formData = this.appendSolucio();
